...
describe('[class] Users select arrange test case;', () => {
  ...
  test('select first arrange column records;', async () => {
    const users = await global.user.tb.select([2, 10], ['name', 'age'], true);
  ...
  test('select third arrange column duplicate records;', async () => {
    const users = await global.user.tb.select([1, 5], ['age', 'gender'], true);
    expect(JSON.stringify(users)).toMatch('[{\"age\":22,\"gender\":1},{\"age\":23,\"gender\":1},{\"age\":32,\"gender\":1},{\"age\":23,\"gender\":1},{\"age\":33,\"gender\":1}]');
  });
  test('select fourth arrange column duplicate records;', async () => {
    const users = await global.user.tb.select([3, 11], ['city', 'country'], true);
  ...
  test('select fifth arrange column duplicate records;', async () => {
