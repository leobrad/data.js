...
describe('[class] Users composite test case;', () => {
  ...
  test('update arbitrarliy record;', async () => {
    const global_user_tb = global.user.tb;
    await global_user_tb.update({ id: 17, age: 18, });
  ...
  test('delete arbitrarliy record;', async () => {
    const global_user_tb = global.user.tb;
    await global_user_tb.delete(20);
  ...
  test('delete exchange arbitrarliy record;', async () => {
    const global_user_tb = global.user.tb;
    await global_user_tb.deleteExchange(16, 19);
    const users = await global_user_tb.select([19, 19]);
    expect(JSON.stringify(users)).toMatch('[]');
  });
});
...
afterAll(async () => {
  const global_user_tb = global.user.tb;
  await global_user_tb.deleteExchange(17, 18);
  await global_user_tb.deleteExchange(18, 17);
  await global_user_tb.deleteExchange(19, 16);
});
